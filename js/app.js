// app.js - полная версия с плавными анимациями и исправленным переключением вкладок
...
let POLL_TIMER = null;
let currentScreen = 'home'; // Запоминаем текущий экран для предотвращения наложений

/* ═══ Навигация (ПЛАВНАЯ, БЕЗ НАЛОЖЕНИЙ) ═══ */
const navItems = document.querySelectorAll(".nav-item");
const screens = document.querySelectorAll(".screen");
const navIndicator = document.getElementById("nav-indicator");

function goTo(target) {
    if (currentScreen === target) return; // Если уже на этом экране — ничего не делаем
    
    // Плавное переключение вкладок (индикатор)
    navItems.forEach(n => n.classList.toggle("active", n.dataset.target === target));
    
    // ⚡ ФИКС: Сначала скрываем ВСЕ экраны, потом показываем нужный
    screens.forEach(s => {
    ...
    currentScreen = target; // Запоминаем текущий экран
    
    // Haptic feedback
    tg.HapticFeedback?.impactOccurred("light");
    
    // Загрузка данных (асинхронно, не блокирует интерфейс)
    if (target === "orders") setTimeout(loadOrders, 50);
    if (target === "admin") setTimeout(loadAdminStats, 50);
}
